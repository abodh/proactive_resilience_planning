...
model.bus_gen_ss = Var(model.bus_i, bounds=(0, Gmax), within=NonNegativeReals, initialize=0)
...
model.bus_gen = Var(model.bus_i, bounds=(0, Gmax), within=NonNegativeReals, initialize=0)
...
# variable indicating the generation curtailment in each of the gen buses
model.gen_curtailment = Var(model.bus_i, bounds=(0, Gmax), within=NonNegativeReals, initialize=0)
...
for bus_num_idx in range(model.nNodes):
    ...
    model.power_balance_ss.add(sum(model.bus_gen_ss[np.where(model.bus[:, 0] == model.gen[gen_num, 0])[0][0]]
                                   for gen_num in gens) +
...
for gen_num in range(model.nGen):
    model.gen_limit_ss.add(model.bus_gen_ss[np.where(model.bus[:, 0] == model.gen[gen_num, 0])[0][0]] <=
...
for bus_num_idx in range(model.nNodes):
    ...
    if not np.any(np.equal(model.gen[:, 0], bus_num)):
        model.gen_limit_ss.add(model.bus_gen_ss[bus_num_idx] == 0)
...
for bus_num_idx in range(model.nNodes):
    ...
    model.power_balance.add(sum(model.bus_gen[np.where(model.bus[:, 0] == model.gen[gen_num, 0])[0][0]] -
                                model.gen_curtailment[np.where(model.bus[:, 0] == model.gen[gen_num, 0])[0][0]]
                                for gen_num in gens) +
...
# col 9: PMAX and col 10: PMIN (Note: in Python number starts from 0)
for gen_num in range(model.nGen):
    model.gen_limit.add(model.bus_gen[np.where(model.bus[:, 0] == model.gen[gen_num, 0])[0][0]] <=
                        model.gen[gen_num, 8] / model.Pbase)
    model.gen_limit.add(model.bus_gen[np.where(model.bus[:, 0] == model.gen[gen_num, 0])[0][0]] >=
                        model.gen[gen_num, 9] / model.Pbase)
...
for bus_num_idx in range(model.nNodes):
    ...
    if not np.any(np.equal(model.gen[:, 0], bus_num)):
        model.gen_limit.add(model.bus_gen[bus_num_idx] == 0)
        model.gen_limit.add(model.gen_curtailment[bus_num_idx] == 0)
...
for gen_num in range(model.nGen):
    if model.gen[gen_num, 16] == 0:
        # if no ramp rate data is given ramp rate is obtained from their capacity
        model.gen[gen_num, 16] = ramp_rate * (model.gen[gen_num, 8]/model.Pbase) * time_for_ramp

    model.ramp_rate.add(model.bus_gen_ss[np.where(model.bus[:, 0] == model.gen[gen_num, 0])[0][0]] -
                        model.bus_gen[np.where(model.bus[:, 0] == model.gen[gen_num, 0])[0][0]]
...
for gen_num in range(model.nGen):
    ...
    model.generation_curtailment_limit.add \
        (model.gen_curtailment[np.where(model.bus[:, 0] == model.gen[gen_num, 0])[0][0]] >= 0)
